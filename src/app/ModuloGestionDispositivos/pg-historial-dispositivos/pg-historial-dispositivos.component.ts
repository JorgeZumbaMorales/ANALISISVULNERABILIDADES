...
@Component({
  ...
  imports: [Toast, ConfirmDialog, PrimeTemplate, ButtonDirective,
    TableModule, IconField, InputIcon, InputText, Tag, NgIf,
    Button, Tooltip, NgClass, Dialog, IftaLabel, FormsModule,
    AutoComplete, Timeline, ProgressSpinnerModule,
    ListboxModule, DrawerModule]
})
export class PgHistorialDispositivosComponent implements OnInit, AfterViewInit {
  ...
  actualizarListbox() {
    const temp = [...this.puertosDispositivo];
    this.puertosDispositivo = [];
    setTimeout(() => {
      this.puertosDispositivo = temp;
    }, 50);
  }
  ...
  seleccionarIP(ip: any, elemento: HTMLElement): void {
    ...
    if (this.ipSeleccionada) {
      setTimeout(() => {
        ...
        setTimeout(() => {
          const contenedor = document.querySelector('#scrollContenedor');
          contenedor?.scrollBy({ top: -50, behavior: 'smooth' });
        }, 150);
      }, 50);
    }
  }
  alCerrarDrawerHistorial(): void {
    this.ipSeleccionada = null;
  }

  cerrarHistorialManual(): void {
    this.ipSeleccionada = null;
